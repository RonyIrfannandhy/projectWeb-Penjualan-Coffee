...
class Product extends MY_Controller
{
    ...
    public function create()
    {
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] !== '') {   // Jika upload'an tidak kosong
            $imageName  = url_title($input->title, '-', true) . '-' . date('YmdHis');  // Membuat menu_coffeeshop
    ...
    public function edit($id)
    {
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] !== '') {   // Jika upload'an tidak kosong
            $imageName  = url_title($data['input']->title, '-', true) . '-' . date('YmdHis');  // Membuat menu_coffeeshop
            $upload     = $this->product->uploadImage('image', $imageName);    // Mulai upload
            if ($upload) {
    ...
    public function unique_slug()
    {
        ...
        $product    = $this->product->where('slug', $slug)->first(); // Akan terisi jika terdapat menu_coffeeshop yang sama

        if ($product) {
            if ($id == $product->id) {  // Keperluan edit tidak perlu ganti menu_coffeeshop, jadi tidak masalah
                return true;
            }

            // Jika terdapat suatu nilai pada $product, berikan pesan error pertanda menu_coffeeshop sudah ada di db
